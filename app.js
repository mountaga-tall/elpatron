(function(){
  ...
  function renderHeader(){
    ...
    const closeMobile=()=>{menu?.classList.remove('open');menu?.setAttribute('aria-hidden','true')};
  ...
  function productCard(p,idx,categoryTitle,categorySlug){
    ...
    const priceMarkup=options.length===1?money(options[0].amount):options.length?'À partir de '+money(Math.min(...options.map(x=>x.amount))):'Sur demande';
    const optionsMarkup=options.length>1?`<div class="option-list">${options.map((o,i)=>`<button type="button" class="option-pill ${i===0?'selected':''}" data-option-index="${i}">${esc(o.label)} · ${money(o.amount)}</button>`).join('')}</div>`:'';
    ...
    return `<article class="product-card reveal tilt" data-product-id="${escAttr(p.id)}"><div class="product-top"><div class="product-num">${String(idx+1).padStart(2,'0')} · EL PATRÓN</div>${p.sub?`<span class="sub-chip">${esc(p.sub)}</span>`:''}<div class="product-name">${esc(p.name)}</div></div><div class="product-body"><p class="product-desc">${esc(p.desc||'Composition selon la recette El Patrón.')}</p>${p.note?`<div class="product-note-inline">${esc(p.note)}</div>`:''}${optionsMarkup}<div class="product-foot"><div class="price" data-price>${priceMarkup}<small>${options.length>1?'Choisissez un format':''}</small></div>${addButton}</div></div></article>`;
  }
  function bindProductButtons(scope,items){
    $$('.product-card',scope).forEach(card=>{
      ...
      let selected=0;
      $$('.option-pill',card).forEach((b,i)=>b.addEventListener('click',()=>{selected=i;$$('.option-pill',card).forEach(x=>x.classList.remove('selected'));b.classList.add('selected');const price=$('[data-price]',card);if(price)price.innerHTML=money(p.price[i].amount)+'<small>'+esc(p.price[i].label)+'</small>'}));
